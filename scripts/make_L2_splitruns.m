function [] = make_L2_splitruns(subj_id, run_ids, dicom_name, exp_name, num_conditions, ...
    block_length, contrast, TR)

% build and save the per-subject l2 structure used by fs-fast for the split-run analysis.
% write the metadata needed by downstream analysis steps.

paths = emfl_paths();
project_dir = paths.project_dir;

filepath_l2 = fullfile(paths.analysis_dir, ['L2_' subj_id '_splitruns.mat']);
...
L2.(exp_name).analysis_dir = fullfile(L2.(exp_name).project_dir, 'analysis', [subj_id '_splitruns']);
